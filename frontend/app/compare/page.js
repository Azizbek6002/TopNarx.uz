...
// THE FIX: always append the array index so every row gets a guaranteed-unique key,
// even when price.id is null AND multiple rows share the same restaurant+price combo.
function normalizePrices(items) {
  const list = normalizeList(items);
  return list.map((price, index) => {
    let stableId;
    if (price.id != null && String(price.id).trim() !== "") {
      // Real DB id exists — still suffix with index so two rows with the same
      // price.id (data bug upstream) never collide here.
      stableId = `id_${String(price.id).trim()}_${index}`;
    } else {
      // No id at all — fall back to as much info as possible + index
      stableId = `fallback_${price.restaurant_id ?? "x"}_${price.price_amount ?? "x"}_${index}`;
    }
    return { ...price, _stableId: stableId };
  });
}
...
function getDistanceText(price) {
  if (price?.distance_km === null || price?.distance_km === undefined)
...
function Spinner() {
  return (
    <svg
      style={{ width: 20, height: 20, animation: "cp-spin 0.8s linear infinite" }}
      fill="none"
      viewBox="0 0 24 24"
    >
...
export default function ComparePage() {
  ...
  async function handleSearch() {
    ...
    try {
      ...
      if (selectedCategory) {
        data = await apiFetch(
          `/api/menu/search/variants/?q=${encodeURIComponent(q)}&category=${encodeURIComponent(selectedCategory)}`,
          { method: "GET", auth: false }
        );
      } else {
        data = await searchVariants(q);
      }
      setSearchResults(dedupeSearchResults(data));
    } catch {
      setSearchResults([]);
    } finally {
  ...
  async function handleSelectItem(item) {
    ...
    try {
      ...
      setVariants(
        normalizeList(
          await apiFetch(
            `/api/menu/items/${encodeURIComponent(itemId)}/variants/`,
            { method: "GET", auth: false }
          )
        )
      );
  ...
  async function handleSelectVariant(variant) {
    ...
    try {
      const raw = await comparePricesByVariant(variant.id);
      // normalizePrices now guarantees every row has a unique _stableId
      setPrices(normalizePrices(raw));
    } catch {
      setPrices([]);
    } finally {
  ...
  /* ── Variant favorite (from search results) ── */
  function handleToggleFavoriteVariant(e, result) {
    e.stopPropagation();
    ...
    setFavorites(
      toggleFavorite({
        ...
        type: "variant",
        variantId: variantId,
        itemName: result.item_name || result.name,
  ...
  /* ── Price favorite — keyed on _stableId which is always unique per row ── */
  function handleToggleFavoritePrice(e, price) {
    e.stopPropagation();
    const favoriteId = `price_${price._stableId}`;
    setFavorites(
      toggleFavorite({
        id: favoriteId,
        type: "price",
        priceId: price._stableId,
        restaurant: price.restaurant_name,
        address: getRestaurantAddress(price),
  ...
  function isVariantFavorite(variantId) {
    return favorites.some(
      (f) => f.type === "variant" && String(f.variantId) === String(variantId)
    );
  }

  // Checks the exact _stableId — no ambiguity possible
  function isPriceFavorite(stableId) {
    return favorites.some(
      (f) => f.type === "price" && String(f.priceId) === String(stableId)
    );
  }
  ...
  return (
    <>
      <style>{`
        ...
        .cp-price-row:last-child { margin-bottom: 0; }
                ...
                <div>
                  ...
                  <h1 className="cp-display" style={{ fontSize: "clamp(28px, 4vw, 48px)", color: "var(--cp-ivory)", marginBottom: 10 }}>
                    ...
                    <em style={{
                      ...
                      background: "linear-gradient(90deg,var(--cp-gold-dk),var(--cp-gold-lt),var(--cp-gold))",
                      ...
                      {searchResults.length > 0 ? (
                        <div className="cp-card cp-scroll" style={{ maxHeight: 480, overflowY: "auto" }}>
                          {searchResults.map((result) => {
                            const variantId = result.id;
                            const isFav = isVariantFavorite(variantId);
                            return (
                              <div
                                key={variantId}
                              ...
                              >
                                ...
                                <button
                                  type="button"
                                  className="cp-star"
                                  style={{ color: isFav ? "var(--cp-gold)" : "var(--cp-ash)" }}
                                  onClick={(e) => handleToggleFavoriteVariant(e, result)}
                    ...
                    {pricesLoading ? (
                      ...
                      <div>
                        {prices.map((price, index) => {
                          // _stableId is always unique per row — no two rows will ever share it
                          const stableId = price._stableId;
                          const isFav = isPriceFavorite(stableId);
                          return (
                            <div
                              key={stableId}
                            ...
                            >
                              ...
                              {/* Star button — scoped to this row's unique stableId */}
                              <button
                                type="button"
                                className="cp-star"
                                style={{ color: isFav ? "var(--cp-gold)" : "var(--cp-ash)" }}
                                onClick={(e) => handleToggleFavoritePrice(e, price)}
                                title={isFav ? "Favoritdan olib tashlash" : "Favoritga qo'shish"}
                              >
